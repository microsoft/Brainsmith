...
if TYPE_CHECKING:
    from ...tools.hw_kernel_gen.rtl_parser.rtl_data import Parameter
    from ...tools.hw_kernel_gen.rtl_parser.pragmas import Pragma
    from ...tools.hw_kernel_gen.metadata import RelationshipMetadata
...
class KernelMetadata:
    ...
    def validate(self) -> List[str]:
        ...
        for interface in self.interfaces:
            logger.debug(f"Validating interface '{interface.name}' (type: {interface.interface_type.value})")
            interface_errors, interface_warnings = interface.validate_parameters(param_names)
            if interface_errors:
                logger.debug(f"Interface '{interface.name}' validation errors: {interface_errors}")
