...
class InputDefinition(BaseDefinition):
    ...
    def create_model(self,
        ...
        if self.datatype_constraints and not self.validates_datatype(datatype):
            ...
            raise ValueError(
                ...
                f"constraints for input '{self.name}'. Valid types: {valid_types}"
            )
    ...
    def derive_stream_dims(self, block_dims: Shape, parameter_binding: Optional[ParameterBinding] = None, config: Optional[Dict[str, Any]] = None) -> Shape:
        """Derive stream dimensions from block dimensions using tiling strategy
        
        Args:
            block_dims: Block dimensions to subdivide into streams
            parameter_binding: Parameter values for SDIM expressions
            config: Runtime configuration
            
        Returns:
            Stream dimensions (SDIM) for each block dimension
            
        Examples:
            # No stream tiling - stream entire block
            block_dims=[32, 16] → stream_dims=[32, 16]
            
            # With stream tiling
            block_dims=[32, 16], stream_tiling=[1, "SIMD"]
            → stream_dims=[1, SIMD_value]
        """
        if not self._tiling_strategy or not self._tiling_strategy.stream_spec:
        ...
        # Apply stream tiling
        result = self._tiling_strategy.apply_stream_tiling(block_dims, params_dict)
        
        # Log warnings if any
        if result.warnings:
            for warning in result.warnings:
                print(f"Warning in {self.name} stream tiling: {warning}")
    ...
    def __repr__(self) -> str:
        ...
        return f"InputDefinition({', '.join(parts)})"