...
class DesignSpaceDefinition:
    ...
    raw_blueprint_config: Dict[str, Any] = field(default_factory=dict)  # NEW: Store original YAML
...
def load_blueprint(blueprint_path: str) -> DesignSpaceDefinition:
    """Load Blueprint from YAML file with enhanced inheritance support."""
    ...
    # Track the final merged data for raw config storage
    merged_data = raw_data.copy()
    
    # Handle inheritance with enhanced system
    if 'base_blueprint' in raw_data and raw_data['base_blueprint']:
        try:
            ...
            base_path = resolve_blueprint_path(blueprint_path, raw_data['base_blueprint'])
            base_blueprint = load_blueprint(str(base_path))
            
            # Merge base with current using intelligent merging
            merged_data = merge_blueprints(base_blueprint, raw_data)
    ...
    blueprint = _parse_blueprint_data(merged_data, str(blueprint_path))
    
    # Store the final merged raw configuration
    blueprint.raw_blueprint_config = merged_data
    ...
    logger.debug(f"Loaded blueprint with raw config preserved: {len(merged_data)} sections")
    return blueprint
