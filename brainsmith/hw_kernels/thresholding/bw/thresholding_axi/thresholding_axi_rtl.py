...
# Date: 2025-06-20T23:09:54.968074
        ...
        for idx, interface in enumerate(interface_metadata):
            ...
            if hasattr(interface, 'bdim_param') and interface.bdim_param:
                if interface.interface_type.value == 'input' and idx < len(folded_input_shape):
                    bdim_value = folded_input_shape[idx][0]  # First dimension is BDIM
                    ...
                    bdim_value = folded_output_shape[idx][0]  # First dimension is BDIM
                    code_gen_dict[f"${interface.bdim_param.upper()}$"] = [str(bdim_value)]
                # For weight interfaces, use input shape as default
                elif interface.interface_type.value == 'weight' and len(folded_input_shape) > 0:
                    bdim_value = folded_input_shape[0][0]  # Use first input's BDIM
                    code_gen_dict[f"${interface.bdim_param.upper()}$"] = [str(bdim_value)]
            
            # Set SDIM parameter value
            if hasattr(interface, 'sdim_param') and interface.sdim_param:
                if interface.interface_type.value == 'input' and idx < len(folded_input_shape):
                    sdim_value = folded_input_shape[idx][1] if len(folded_input_shape[idx]) > 1 else 1
                    code_gen_dict[f"${interface.sdim_param.upper()}$"] = [str(sdim_value)]
                elif interface.interface_type.value == 'output' and idx < len(folded_output_shape):
                    sdim_value = folded_output_shape[idx][1] if len(folded_output_shape[idx]) > 1 else 1
                    code_gen_dict[f"${interface.sdim_param.upper()}$"] = [str(sdim_value)]
                # For weight interfaces, use input shape as default
                elif interface.interface_type.value == 'weight' and len(folded_input_shape) > 0:
                    sdim_value = folded_input_shape[0][1] if len(folded_input_shape[0]) > 1 else 1
                    code_gen_dict[f"${interface.sdim_param.upper()}$"] = [str(sdim_value)]
