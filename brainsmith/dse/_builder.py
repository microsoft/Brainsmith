...
class DSETreeBuilder:
    ...
    def build_tree(self, space: GlobalDesignSpace, blueprint_config: DSEConfig) -> DSETree:
        ...
        """
        root = DSESegment(steps=[], finn_config=self._extract_finn_config(space, blueprint_config))
    ...
    def _extract_finn_config(
        self, space: GlobalDesignSpace, blueprint_config: DSEConfig
    ) -> dict[str, Any]:
        ...
        for kernel_name, backend_classes in space.kernel_backends:
            ...
            if backend_classes:
                ...
                if hasattr(first_backend, "__registry_name__"):
                    ...
                    backend_meta = get_component_metadata(
                        first_backend.__registry_name__, "backend"
                    )
                    ...
                    qualified_kernel_name = (
                        kernel_name if ":" in kernel_name else f"brainsmith:{kernel_name}"
                    )
    ...
    def _create_branches(
        self, segments: list[DSESegment], branch_index: int, branch_options: list[str]
    ) -> list[DSESegment]:
        new_segments = []
