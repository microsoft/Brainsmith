...
class LayerNorm(AutoHWCustomOp):
    ...
    def make_shape_compatible_op(self, model):
        """Create shape-compatible Identity operation for InferShapes transformation.

        Overrides the default AutoHWCustomOp behavior (which returns RandomNormal)
        to return an Identity node. LayerNorm preserves the input shape (output shape
        equals input shape), so Identity is semantically correct for shape inference.

        Note: This is called during InferShapes BEFORE tensor context initialization,
        so we use a simple shape-preserving operation that ONNX understands natively.

        Args:
            model: ModelWrapper instance

        Returns:
            Identity node with same inputs/outputs as this node
        """
        # LayerNorm is shape-preserving (output shape = input shape)
        # Use Identity node for ONNX shape inference
        return make_node(
            "Identity",
