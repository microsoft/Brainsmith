...
class KernelGenerator:
    ...
    ARTIFACTS = {
        "kernelop": {"template": "auto_hw_custom_op.py.j2", "filename": "{name}.py"},
        "rtlbackend": {"template": "auto_rtl_backend.py.j2", "filename": "{name}_rtl.py"},
        "wrapper": {"template": "rtl_wrapper.v.j2", "filename": "{name}_wrapper.v"},
        "init": {"template": "__init__.py.j2", "filename": "__init__.py"},
    }
    ...
    def generate(
        self, artifact_type: str, kernel_metadata: KernelMetadata, output_dir: Path
    ) -> Path:
