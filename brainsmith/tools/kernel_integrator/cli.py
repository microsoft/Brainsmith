...
Notes:
  RTL files should contain @brainsmith pragmas to define interfaces and parameters.
    ...
    mode_group.add_argument(
        "--validate", action="store_true", help="validate RTL only without generating files"
    )
    mode_group.add_argument(
        "--info",
        action="store_true",
    ...
    parser.add_argument("--no-strict", action="store_true", help="disable strict validation")
...
def parse_rtl_file(rtl_file: Path) -> KernelMetadata:
    ...
    try:
        ...
        logger.info(
            f"Successfully parsed RTL file {rtl_file} → KernelMetadata '{parsed_data.name}'"
        )
        return parsed_data
...
def parse_artifacts_list(artifacts_str: str) -> list[str]:
    ...
    if invalid:
        raise ValueError(
            f"Invalid artifacts: {', '.join(invalid)}. Valid options: {', '.join(valid_artifacts)}"
        )
