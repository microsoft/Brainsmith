...
class RTLParser:
    ...
    def parse(self, systemverilog_code: str, source_name: str = "<string>") -> KernelMetadata:
        ...
        try:
            ...
            parsed_module = self.module_extractor.extract_from_tree(tree, source_name)
            ...
            logger.info(
                f"KernelMetadata object created for '{kernel_metadata.name}' with {len(kernel_metadata.parameters)} params"
            )
            logger.info(
                f"Successfully parsed and processed module '{kernel_metadata.name}' from {source_name}"
            )
