...
class BDimPragma(InterfacePragma):
        ...
        for element in bdim_params:
            if isinstance(element, str) and element != "1" and element.isidentifier():
                ...
                if element not in param_names:
                    error_msg = (
                        f"BDIM pragma at line {self.inputs.get('line_number', 'unknown')} references unknown parameter '{element}'. "
    ...
    def _validate_shape_expression(self, shape_expr: list[str], expected_length: int) -> list:
        ...
        if len(shape_expr) != expected_length:
            raise PragmaError(
                f"SHAPE length {len(shape_expr)} does not match parameter count {expected_length}"
            )
        ...
        for i, element in enumerate(shape_expr):
            if element == "1":
                ...
                raise PragmaError(
                    f"Invalid SHAPE element '{element}' at position {i}. "
    ...
    def _parse_inputs(self) -> dict:
        ...
        """
        logger.debug(
            f"Parsing BDIM pragma: {self.inputs} at line {self.inputs.get('line_number', 'unknown')}"
        )
        ...
        if not interface_name.replace("_", "").replace("V", "").isalnum():
            raise PragmaError(
                f"BDIM pragma interface name '{interface_name}' contains invalid characters"
            )
        ...
        if isinstance(pos[1], list):
            ...
            for element in bdim_params:
                if element == "1":
                    ...
                    raise PragmaError(
                        f"Magic numbers not allowed in BDIM pragma except '1'. Use parameter names instead of '{element}'."
                    )
                elif element.isidentifier():
                    has_real_param = True
                else:
                    raise PragmaError(
                        f"Invalid parameter '{element}'. Must be '1' (singleton) or parameter name."
                    )

            # Ensure at least one real parameter if using singletons
            if not has_real_param:
                raise PragmaError(
                    "BDIM pragma list must contain at least one parameter name (not just '1's)"
                )
        else:
            ...
            if param == "1":
                raise PragmaError(
                    "Single BDIM parameter cannot be '1'. Use a parameter name or list syntax [1, param] for singleton dimensions."
                )
        ...
        if len(pos) > 2:
            raise PragmaError(
                f"Unexpected extra arguments in BDIM pragma: {pos[2:]}. RINDEX is no longer supported."
            )
    ...
    def apply_to_kernel(self, kernel: KernelMetadata) -> None:
        ...
        if hasattr(interface, "interface_type"):
            ...
            if interface.interface_type == InterfaceType.CONFIG:
                if not (hasattr(interface, "is_weight") and interface.is_weight):
                    error_msg = (
                        f"BDIM pragma at line {self.inputs.get('line_number', 'unknown')} cannot be applied to "
        ...
        for param_ref in bdim_params:
            ...
            if param_index is not None:
                ...
                logger.debug(
                    f"Moved parameter '{param.name}' from kernel to interface '{interface_name}' dataflow.bdim_params"
                )
            else:
                logger.warning(
                    f"BDIM pragma references parameter '{param_ref}' which is not in kernel.parameters"
                )

        # Apply the shape to the interface's dataflow metadata
        interface.dataflow.bdim_shape = bdim_shape
        logger.debug(
            f"BDIM pragma successfully applied to interface '{interface_name}' with shape={bdim_shape}"
        )
...
class SDimPragma(InterfacePragma):
    ...
    def validate_with_kernel(self, kernel: KernelMetadata) -> None:
        ...
        for param in sdim_params:
            if param != "1" and param not in param_names:
                error_msg = (
                    f"SDIM pragma at line {self.inputs.get('line_number', 'unknown')} references unknown parameter '{param}'. "
                    f"Available parameters: {sorted(param_names) if param_names else 'none'}"
                )
    ...
    def _parse_inputs(self) -> dict:
        ...
        """
        logger.debug(
            f"Parsing SDIM pragma: {self.inputs} at line {self.inputs.get('line_number', 'unknown')}"
        )
        ...
        if len(pos) < 2:
            raise PragmaError(
                "SDIM pragma requires at least two arguments: interface name and parameter(s)"
            )
        ...
        if not interface_name.replace("_", "").replace("V", "").isalnum():
            raise PragmaError(
                f"SDIM pragma interface name '{interface_name}' contains invalid characters"
            )
        ...
        if isinstance(pos[1], list):
            ...
            for element in sdim_params:
                if element == "1":
                    ...
                    raise PragmaError(
                        f"Magic numbers not allowed in SDIM pragma except '1'. Use parameter names instead of '{element}'."
                    )
                elif element.isidentifier():
                    has_real_param = True
                else:
            ...
            if not has_real_param:
                raise PragmaError(
                    "SDIM pragma list must contain at least one parameter name (not just '1's)"
                )
        else:
            # Single parameter - no brackets
            param = pos[1]
            if param == "1":
                raise PragmaError(
                    "Single SDIM parameter cannot be '1'. Use a parameter name or list syntax [1, param] for singleton dimensions."
                )
            elif not param.isidentifier():
    ...
    def apply_to_kernel(self, kernel: KernelMetadata) -> None:
        ...
        if hasattr(interface, "interface_type"):
            ...
            if interface.interface_type == InterfaceType.CONFIG:
                if not (hasattr(interface, "is_weight") and interface.is_weight):
                    error_msg = (
                        f"SDIM pragma at line {self.inputs.get('line_number', 'unknown')} cannot be applied to "
                        f"CONFIG interface '{interface_name}' unless it is marked as a weight. "
                        f"Use '@brainsmith weight {interface_name}' first."
                    )
        ...
        for param_ref in sdim_params:
            ...
            if param_index is not None:
                ...
                logger.debug(
                    f"Moved parameter '{param.name}' from kernel to interface '{interface_name}' dataflow.sdim_params"
                )
            else:
                logger.warning(
                    f"SDIM pragma references parameter '{param_ref}' which is not in kernel.parameters"
                )

        # Apply the shape to the interface's dataflow metadata
        interface.dataflow.sdim_shape = sdim_shape
        logger.debug(
            f"SDIM pragma successfully applied to interface '{interface_name}' with shape={sdim_shape}"
        )
