...
class DatatypeConstraintPragma(InterfacePragma):
    ...
    def _parse_inputs(self) -> dict:
        ...
        """
        logger.debug(f"Parsing DATATYPE_CONSTRAINT pragma: {self.inputs} at line {self.inputs.get('line_number', 'unknown')}")
        
        pos = self.inputs['positional']
        
        if len(pos) != 4:
            raise PragmaError("DATATYPE_CONSTRAINT pragma requires interface_name, base_type(s), min_bits, max_bits")
        ...
        try:
            ...
            raise PragmaError(f"DATATYPE_CONSTRAINT pragma min_bits and max_bits must be integers, got: {pos[2]}, {pos[3]}")
        
        if min_bits <= 0:
            raise PragmaError(f"DATATYPE_CONSTRAINT pragma min_bits must be positive, got: {min_bits}")
        
        if min_bits > max_bits:
            raise PragmaError(f"DATATYPE_CONSTRAINT pragma min_bits ({min_bits}) cannot be greater than max_bits ({max_bits})")
        ...
        for base_type in base_types:
            ...
            try:
                ...
                raise PragmaError(f"DATATYPE_CONSTRAINT pragma invalid base type '{base_type}' or constraints: {e}")
    ...
    def apply_to_kernel(self, kernel: KernelMetadata) -> None:
        ...
        if interface is None:
            logger.warning(f"DATATYPE_CONSTRAINT pragma target interface '{interface_name}' not found")
            return
        ...
        if hasattr(interface, 'interface_type') and interface.interface_type == InterfaceType.CONTROL:
            error_msg = (f"DATATYPE_CONSTRAINT pragma at line {self.inputs.get('line_number', 'unknown')} cannot be applied to "
        ...
        logger.debug(f"DATATYPE_CONSTRAINT pragma successfully applied to interface '{interface_name}' with {len(new_constraint_groups)} constraint groups")
...
class DatatypePragma(InterfacePragma):
    ...
    def _parse_inputs(self) -> dict:
        ...
        if len(pos) != 3:
            raise PragmaError("DATATYPE pragma requires interface_name, property_type, parameter_name")
    ...
    def apply_to_kernel(self, kernel: 'KernelMetadata') -> None:
        ...
        if param_index is not None:
            ...
            logger.warning(f"DATATYPE pragma references parameter '{parameter_name}' which is not in kernel.parameters")
