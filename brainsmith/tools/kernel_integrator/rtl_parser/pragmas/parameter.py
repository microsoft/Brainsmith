...
class AliasPragma(Pragma):
    ...
    def _parse_inputs(self) -> dict:
        ...
        logger.debug(
            f"Parsing ALIAS pragma: {self.inputs} at line {self.inputs.get('line_number', 'unknown')}"
        )
        ...
        if len(pos) != 2:
            raise PragmaError(
                f"ALIAS pragma at line {self.inputs.get('line_number', 'unknown')} requires exactly 2 arguments: <rtl_param> <nodeattr_name>. Got: {len(pos)} arguments"
            )
        ...
        if not rtl_param.isidentifier():
            raise PragmaError(
                f"ALIAS pragma RTL parameter name '{rtl_param}' is not a valid identifier"
            )
        if not nodeattr_name.isidentifier():
            raise PragmaError(
                f"ALIAS pragma nodeattr name '{nodeattr_name}' is not a valid identifier"
            )
    ...
    def apply_to_kernel(self, kernel: "KernelMetadata") -> None:
        ...
        if not found:
            logger.warning(
                f"ALIAS pragma references parameter '{rtl_param}' which is not in kernel.parameters"
            )
...
class DerivedParameterPragma(Pragma):
    ...
    def _parse_inputs(self) -> dict:
        ...
        logger.debug(
            f"Parsing DERIVED_PARAMETER pragma: {self.inputs} at line {self.inputs.get('line_number', 'unknown')}"
        )
        ...
        if len(pos) < 2:
            raise PragmaError(
                f"DERIVED_PARAMETER pragma at line {self.inputs.get('line_number', 'unknown')} requires parameter name and Python expression. Got: {len(pos)} arguments"
            )
        ...
        if not param_name.isidentifier():
            raise PragmaError(
                f"DERIVED_PARAMETER pragma parameter name '{param_name}' is not a valid identifier"
            )
    ...
    def apply_to_kernel(self, kernel: "KernelMetadata") -> None:
        ...
        derived_param = Parameter(name=param_name, kernel_value=python_expression)
...
class AxiLiteParamPragma(Pragma):
    ...
    def _parse_inputs(self) -> dict:
        ...
        logger.debug(
            f"Parsing AXILITE_PARAM pragma: {self.inputs} at line {self.inputs.get('line_number', 'unknown')}"
        )
        ...
        if len(pos) != 3:
            raise PragmaError(
                f"AXILITE_PARAM pragma at line {self.inputs.get('line_number', 'unknown')} requires exactly 3 arguments: <param_name> <interface_name> <property>. Got: {len(pos)} arguments"
            )
        ...
        if not param_name.isidentifier():
            raise PragmaError(
                f"AXILITE_PARAM pragma parameter name '{param_name}' is not a valid identifier"
            )

        # Validate interface name
        if not interface_name.replace("_", "").isalnum():
            raise PragmaError(
                f"AXILITE_PARAM pragma interface name '{interface_name}' contains invalid characters"
            )
        ...
        if property_type not in valid_properties:
            raise PragmaError(
                f"Invalid property '{property_type}'. Must be one of: {valid_properties}"
            )
    ...
    def apply_to_kernel(self, kernel: "KernelMetadata") -> None:
        ...
        if param_index is not None:
            ...
            logger.debug(
                f"Moved parameter '{param.name}' from kernel to AXI-Lite interface '{interface_name}' {property_type}_param"
            )
        else:
            logger.warning(
                f"AXILITE_PARAM pragma references parameter '{param_name}' which is not in kernel.parameters"
            )
