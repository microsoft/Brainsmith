...
class RelationshipPragma(Pragma):
    ...
    def _parse_inputs(self) -> dict[str, Any]:
        ...
        if rel_type == "EQUAL":
            ...
            if len(pos) < 6:
                raise PragmaError(
                    "DEPENDENT relationship requires source_dim, target_dim, and dependency_type"
                )
            ...
            if result["dependency_type"] not in valid_dep_types:
                raise PragmaError(
                    f"Invalid dependency type '{result['dependency_type']}'. "
            ...
            if result["dependency_type"] == "scaled":
                if len(pos) < 7:
                    raise PragmaError(
                        "DEPENDENT relationship with 'scaled' type requires scale factor"
                    )
    ...
    def apply_to_kernel(self, kernel: KernelMetadata) -> None:
        ...
        if source not in interface_names:
            raise PragmaError(
                f"Source interface '{source}' not found in kernel. "
                f"Available interfaces: {interface_names}"
            )

        if target not in interface_names:
            raise PragmaError(
                f"Target interface '{target}' not found in kernel. "
                f"Available interfaces: {interface_names}"
            )
        ...
        logger.debug(
            f"Added {relationship.relation.value} relationship between "
