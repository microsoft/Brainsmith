...
class ModuleExtractor:
    ...
    def extract_from_tree(self, tree: Tree, source_name: str = "<string>") -> ParsedModule:
        ...
        module_node = self.select_target_module(module_nodes, pragmas, source_name)
    ...
    def extract_module_header(
        self, module_node: Node
    ) -> tuple[str | None, list[Node] | None, list[Node] | None]:
        ...
        if not module_node or module_node.type != "module_declaration":
            logger.error(
                "Invalid node passed to extract_module_header. Expected 'module_declaration'."
            )
        ...
        header_node = self.ast_parser.find_child(
            module_node, ["module_ansi_header", "module_nonansi_header"]
        )
        ...
        if name_node:
            ...
            logger.warning(
                f"Could not find module name identifier within node: {module_node.text.decode()[:50]}..."
            )

        # Search for parameter and port lists
        logger.debug(
            f"Searching for parameter/port lists within node type: {search_parent_node.type}"
        )
        ...
        if param_list_node:
            param_nodes = self.ast_parser.find_children(
                param_list_node, ["parameter_port_declaration"]
            )
        ...
        port_list_node = self.ast_parser.find_child(
            search_parent_node, ["list_of_port_declarations"]
        )
        if port_list_node:
            ...
            logger.debug(
                "No ANSI port list node found. Non-ANSI port extraction not yet implemented."
            )
    ...
    def parse_parameter_declaration(self, node: Node) -> Parameter | None:
        ...
        param_decl_node = self.ast_parser.find_child(
        ...
        if not param_decl_node:
            if node.type == "local_parameter_declaration":
                ...
                logger.warning(
                    f"Could not find parameter_declaration or local_parameter_declaration within: {node.text.decode()}"
                )
                param_decl_node = node
        ...
        logger.debug(
            f"--- Entering parse_parameter_declaration for node: {param_decl_node.type} | Text: '{param_decl_node.text.decode()[:60]}...'"
        )

        # Extract Type
        param_type = None
        logger.debug("--- Starting type extraction ---")
        type_node = self.ast_parser.find_child(
            param_decl_node, ["data_type_or_implicit", "data_type"]
        )
        ...
        if type_node:
            ...
            type_param_decl = self.ast_parser.find_child(
                param_decl_node, ["type_parameter_declaration"]
            )
            logger.debug(
                f"Found type_param_decl: {type_param_decl.type if type_param_decl else 'None'}"
            )
            if type_param_decl:
                param_type = "type"
        ...
        assignment_list_node = self.ast_parser.find_child(
            param_decl_node, ["list_of_param_assignments"]
        )
        if assignment_list_node:
            assignment_node = self.ast_parser.find_child(assignment_list_node, ["param_assignment"])
            if assignment_node:
                # Extract name
                name_node = self.ast_parser.find_child(
                    assignment_node, ["simple_identifier", "identifier"]
                )
                if name_node:
                    param_name = name_node.text.decode("utf8").strip()
                else:
                    logger.warning(
                        f"Could not find parameter name in assignment: {assignment_node.text.decode()}"
                    )
                    return None

                # Extract default value
                value_expr_node = self.ast_parser.find_child(
                    assignment_node,
                    ["constant_param_expression", "constant_expression", "expression"],
                )
                if value_expr_node:
                    inner_expr = self.ast_parser.find_child(
                        value_expr_node,
                        [
                            "constant_min_type_max_expression",
                            "constant_expression",
                            "primary_literal",
                            "binary_expression",
                        ],
                ...
                logger.warning(
                    f"Could not find param_assignment within list: {assignment_list_node.text.decode()}"
                )
                return None
        else:
            logger.debug(
                f"No list_of_param_assignments found in: {param_decl_node.text.decode()[:50]}..."
            )
            # Handle 'parameter type' declarations
            if param_type == "type":
                logger.debug(
                    f"Handling 'parameter type' specific structure: {param_decl_node.text.decode()[:50]}..."
                )
                type_param_decl_node = self.ast_parser.find_child(
                    param_decl_node, ["type_parameter_declaration"]
                )
                if type_param_decl_node:
                    list_of_assignments = self.ast_parser.find_child(
                        type_param_decl_node, ["list_of_type_assignments"]
                    )
                    if list_of_assignments:
                        assignment_node = self.ast_parser.find_child(
                            list_of_assignments, ["type_assignment"]
                        )
                        if assignment_node:
                            # Extract name
                            name_node = self.ast_parser.find_child(
                            ...
                            if name_node:
                                ...
                                logger.warning(
                                    f"Could not find parameter name in type_assignment: {assignment_node.text.decode()}"
                                )
                                return None
                            # Extract default value
                            ...
                            if value_node:
                                ...
                                logger.debug(
                                    f"Type Parameter '{param_name}' default type found: {default_value}"
                                )
                        else:
                            logger.warning(
                                f"Could not find type_assignment within list: {list_of_assignments.text.decode()}"
                            )
                            return None
                    else:
                        logger.warning(
                            f"Could not find list_of_type_assignments within type_parameter_declaration: {type_param_decl_node.text.decode()}"
                        )
                        return None
                else:
                    logger.warning(
                        f"param_type is 'type' but could not find type_parameter_declaration node within: {param_decl_node.text.decode()}"
                    )
                    return None
            else:
                # Fallback: Try finding name directly
                name_node = self.ast_parser.find_child(
                    param_decl_node, ["simple_identifier", "identifier"]
                )
                if name_node:
                    param_name = name_node.text.decode("utf8").strip()
                    ...
                    if param_type is not None:
                        logger.warning(
                            f"Parameter '{param_name}' has type '{param_type}' but no assignment list found?"
                        )
                else:
                    logger.warning(
                        f"Could not determine parameter name: {param_decl_node.text.decode()}"
                    )
                    return None
        ...
        if param_name:
            ...
            logger.info(
                f"Successfully parsed parameter: Name='{param_name}', Type='{final_param_type}', Default='{default_value}', Line={line_number}"
            )
            return Parameter(
                name=param_name,
            ...
            logger.error(
                f"Failed to extract parameter details from node: {param_decl_node.text.decode()}"
            )
            return None
    ...
    def parse_port_declaration(self, node: Node) -> list[Port]:
        ...
        if variable_port_header:
            ...
            direction = self._extract_direction(
                self.ast_parser.find_child(variable_port_header, ["port_direction"])
            )
            variable_port_type = self.ast_parser.find_child(
                variable_port_header, ["variable_port_type"]
            )
            if variable_port_type:
                ...
                if dt_node:
                    ...
                    # Search for width
                    width_node = self.ast_parser.find_child(
                        dt_node, ["packed_dimension", "unpacked_dimension"]
                    )
                    if not width_node:
                        ...
                        if sibling and sibling.type in ["packed_dimension", "unpacked_dimension"]:
                            ...
                            if sibling and sibling.type in [
                                "packed_dimension",
                                "unpacked_dimension",
                            ]:
                                width_node = sibling
                if not width_node:
                    width_node = self.ast_parser.find_child(
                        variable_port_type, ["packed_dimension", "unpacked_dimension"]
                    )

        elif net_port_header:
            logger.debug("Parsing as Net Port Header")
            direction = self._extract_direction(
                self.ast_parser.find_child(net_port_header, ["port_direction"])
            )
            ...
            if net_port_type:
                ...
                if dtoi_node:
                    ...
                    if idt_node:
                        width_node = self.ast_parser.find_child(
                            idt_node, ["packed_dimension", "unpacked_dimension"]
                        )
                ...
                if not width_node:
                    width_node = self.ast_parser.find_child(
                        net_port_type, ["packed_dimension", "unpacked_dimension"]
                    )
            ...
            if_identifier_node = self.ast_parser.find_child(
                interface_port_header, ["interface_identifier"]
            )
            if if_identifier_node:
                data_type = if_identifier_node.text.decode("utf8").strip()
                modport_node = self.ast_parser.find_child(
                    interface_port_header, ["modport_identifier"]
                )
                if modport_node:
        ...
        if width_node and not interface_port_header:
            logger.debug(
                f"Found potential width node: Type={width_node.type}, Text='{width_node.text.decode()}'"
            )
            ...
            if extracted:
                ...
                logger.warning(
                    f"Width extraction returned empty for node: {width_node.text.decode()}, keeping default '1'."
                )
        elif not interface_port_header:
            logger.debug(f"No width node found. Final width: {final_width}")

        # Extract Port Name(s)
        list_of_ids_node = self.ast_parser.find_child(
            node, ["list_of_port_identifiers", "list_of_variable_identifiers"]
        )
        if list_of_ids_node:
            ...
            for child in reversed(node.children):
                ...
                if (
                    ...
                    and child.prev_sibling.type == "simple_identifier"
                ):
        ...
        if not port_names:
            logger.warning(
                f"Failed to extract any valid port names from node: {node.text.decode()}"
            )
            return []
        ...
        for name in port_names:
            logger.info(
                f"Successfully parsed port: Name='{name}', Direction='{direction.value}', Width='{final_width}', Type='{data_type}'"
            )
    ...
    def _extract_width_from_dimension(self, width_node: Node) -> str:
        ...
        logger.debug(
            f"Extracting width from node: Type={width_node.type}, Text='{width_node.text.decode()}'"
        )

        # Find the expression node within the dimension
        expr_node = self.ast_parser.find_child(
            width_node,
            [
                "constant_range",
                "range_expression",
        ...
        if expr_node:
            logger.debug(
                f"Found expression node: Type={expr_node.type}, Text='{expr_node.text.decode()}'"
            )
            ...
            if full_node_text.startswith("[") and full_node_text.endswith("]"):
                ...
                if width_text == expected_inner_text:
                    ...
                    logger.debug(
                        f"Expression node text ('{width_text}') differs from node inner text ('{expected_inner_text}'), using inner text."
                    )
    ...
    def _find_identifiers_recursive(self, node: Node) -> list[str]:
        ...
        if (
            node_type in ["simple_identifier", "identifier", "port_identifier"]
            and node_text not in keywords_to_exclude
        ):
            # Check parent type to avoid module names
            if not (
                node.parent
                and node.parent.type
                in ["module_declaration", "module_identifier", "interface_identifier"]
            ):
    ...
    def select_target_module(
        self, module_nodes: list[Node], pragmas: list[Pragma], source_name: str
    ) -> Node:
        ...
        for node in module_nodes:
            ...
            if name:
                ...
                logger.warning(
                    f"Could not extract module name from node: {node.text.decode()[:50]}..."
                )
        ...
        # Priority 2: Multiple modules (requires TOP_MODULE pragma)
            ...
            if len(top_module_pragmas) == 1:
                ...
                raise ValueError(
                    f"Multiple TOP_MODULE pragmas found in {source_name}. " f"Only one is allowed."
                )
            else:
    ...
    def extract_pragmas(self, root_node: Node) -> list[Pragma]:
        ...
        def find_comments(node: Node):
            ...
            if node.type == "comment":
                ...
                logger.debug(
                    f"Found 'comment' node at line {node.start_point[0]+1}: {node.text.decode('utf8')[:60]}..."
                )
        ...
        logger.info(
            f"<<< Finished pragma extraction. Found {comments_found_count} comment nodes and {len(pragmas)} valid pragmas."
        )
        return pragmas
    ...
    def _validate_pragma(self, node: Node, line_number: int) -> Pragma | None:
        ...
        if len(parts) > 2:
            ...
            parsed_inputs = {"raw": [], "positional": [], "named": {}}
        ...
        if pragma_enum_type is None or pragma_enum_type not in self.pragma_constructors:
            logger.debug(
                f"Ignoring comment at line {line_number}: Unknown or unsupported pragma type '@brainsmith {pragma_type_str}'"
            )
            return None
        ...
        try:
            ...
            return pragma_class(type=pragma_enum_type, inputs=parsed_inputs)
        except PragmaError as e:
            logger.warning(
                f"Error instantiating pragma {pragma_enum_type.name} at line {line_number}: {e}"
            )
            return None
        except Exception as e:
            logger.error(
                f"Unexpected error instantiating pragma {pragma_enum_type.name} at line {line_number}: {e}"
            )
            return None
