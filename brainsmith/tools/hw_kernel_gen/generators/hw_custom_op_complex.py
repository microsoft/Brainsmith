...
class HWCustomOpGenerator(GeneratorBase):
    ...
    def _get_template_context(self, parsed_data: KernelMetadata) -> dict:
        ...
        context.update({
            'class_name': f"{class_name}HWCustomOp",
            'verification_required': context['verification_required'],
            'kernel_verifications': self._get_kernel_verifications(parsed_data),
            'resource_estimation': self._get_resource_estimation_context(parsed_data)
        })
        
        # Advanced BDIM context if available
        has_enhanced_bdim = len(parsed_data.pragmas) > 0
        if has_enhanced_bdim:
            context.update({
                'interface_metadata': self._build_enhanced_interface_metadata(parsed_data),
                'chunking_context': self._build_chunking_context(parsed_data),
    ...
    def _get_kernel_verifications(self, parsed_data: KernelMetadata) -> list:
        ...
        # Get kernel type using template context generator logic
        kernel_type = self._infer_kernel_type(parsed_data.name)
        ...
        elif kernel_type in ['matmul', 'conv']:
        ...
        has_enhanced_bdim = len(parsed_data.pragmas) > 0
        if has_enhanced_bdim:
            verifications.extend(self._get_bdim_verifications(parsed_data))
        
        # Interface-specific verifications
        verifications.extend(self._get_interface_verifications(parsed_data))
        
        # Parallelism and performance verifications
        if has_enhanced_bdim:
            verifications.extend(self._get_parallelism_verifications(parsed_data))
        
        return verifications
    
    def _infer_kernel_type(self, name: str) -> str:
