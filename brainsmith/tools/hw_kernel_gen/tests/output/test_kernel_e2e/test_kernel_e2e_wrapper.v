...
// Date: 2025-06-20T20:48:28.046590

module test_kernel_e2e_wrapper #(    // General algorithm parameters
    parameter INPUT_BDIM = $INPUT_BDIM$,    parameter INPUT_SDIM = $INPUT_SDIM$,    parameter WEIGHT_BDIM = $WEIGHT_BDIM$,    parameter MEM_DEPTH = $MEM_DEPTH$,    parameter ACTIVATION_TYPE = $ACTIVATION_TYPE$,    // Internal datatype parameters    // accumulator datatype    parameter ACC_WIDTH = $ACC_WIDTH$,    parameter ACC_SIGNED = $ACC_SIGNED$,    // threshold datatype    parameter THRESH_WIDTH = $THRESH_WIDTH$,    // OUTPUT datatype    parameter OUTPUT_WIDTH = $OUTPUT_WIDTH$,    // Interface datatype parameters
    // s_axis_input interface parameters
    parameter INPUT_WIDTH = $INPUT_WIDTH$,    // s_axis_weights interface parameters
    parameter WEIGHT_WIDTH = $WEIGHT_WIDTH$,    parameter WEIGHT_SIGNED = $WEIGHT_SIGNED$) (
    // Global Control
    input wire ap_clk,
    input wire ap_rst_n,
    input wire ap_start,
    ...
    test_kernel_e2e #(        // General algorithm parameters
        .INPUT_BDIM(INPUT_BDIM),        .INPUT_SDIM(INPUT_SDIM),        .WEIGHT_BDIM(WEIGHT_BDIM),        .MEM_DEPTH(MEM_DEPTH),        .ACTIVATION_TYPE(ACTIVATION_TYPE),        // Internal datatype parameters        // accumulator datatype        .ACC_WIDTH(ACC_WIDTH),        .ACC_SIGNED(ACC_SIGNED),        // threshold datatype        .THRESH_WIDTH(THRESH_WIDTH),        // OUTPUT datatype        .OUTPUT_WIDTH(OUTPUT_WIDTH),        // Interface datatype parameters
        // s_axis_input interface parameters
        .INPUT_WIDTH(INPUT_WIDTH),        // s_axis_weights interface parameters
        .WEIGHT_WIDTH(WEIGHT_WIDTH),        .WEIGHT_SIGNED(WEIGHT_SIGNED)) test_kernel_e2e_inst (
        // Global control
        .ap_clk(ap_clk),
        .ap_rst_n(ap_rst_n),
        .ap_start(ap_start),
