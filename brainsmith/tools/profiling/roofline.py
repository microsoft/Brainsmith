...
def print_pipeline(compute, weights, activations, hbm, dtype):
    ...
    activations = [
        [auto_format_with_units(x, "memory") for x in segment] for segment in activations
    ]
...
def roofline_analysis(model, hw_params, dtypes):
    ...
    if model["arch"] == "twin_bert":
        ...
        if "chunk_size" in model.keys():
            if DEBUG:
                print(
                    f"Increasing cycles by {model['seq_len']/model['chunk_size']}x ({model['seq_len']}/{model['chunk_size']})"
                )
...
def define_hardware(model, profile, hw_params, dtype):
    ...
    x_macs_per_sec = (
        x_dsps * hw_params["dsp_hz"] * macs_per_dsp[dtype]
        + x_luts * hw_params["lut_hz"] / luts_per_mac[dtype]
    )
    ...
    e2e_latency = profile["cycles"] * (4 * overhang_latency + 2 * x_latency) + (
        x_latency - overhang_latency
    )
    ...
    if hbm_ratio > 1:
        ...
        print(
            f"HBM Bottleneck! Throttling IPS & Latency by {hbm_ratio:.6f}x ({theoretical_ips:.6f} --> {ips:.6f})"
        )
    ...
    print(
        f"    HBM: {hbm_bandwidth_cycles/8e9:.2f} + {hbm_bandwidth_spill/8e9:.2f} = {hbm_bandwidth/8e9:.2f}/{int(hbm/8e9)} GB/s = {hbm_bandwidth/hbm:.2f}x"
    )
    print(
        f"    SRAM: {sram_weights:.2f} weights + {sram_act:.2f} activations = {sram_used:.2f}/{sram/8e6:.2f} MB"
    )
