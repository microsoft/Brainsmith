...
class RooflineModel:
    ...
    def update_model(self, model):
        ...
        self.kv_heads = (
            model["num_kv_heads"] if "num_kv_head" in model.keys() else model["num_heads"]
        )
        ...
        self.spill_map = (
            model["spill_map"] if "spill_map" in model.keys() else PIPELINE_LENGTH * [False]
        )
        self.residuals_in_hbm = (
            model["residuals_in_hbm"] if "residuals_in_hbm" in model.keys() else False
        )
        self.attn_kernel_fusion = (
            model["attn_kernel_fusion"] if "attn_kernel_fusion" in model.keys() else False
        )
        # Iterations
    ...
    def get_profile(self):
        profile = {
            "act": self.iterations * self.activations,
    ...
    def profile_qkv(self, seq_len, embed_dim, q_heads, k_heads, v_heads, head_size, pos=0):
        ...
        q_compute = (
            seq_len * embed_dim * (q_heads * head_size)
        )  # [seq, embed] x [embed, q_heads*head_dim] -> [seq, q_heads*head_dim]
        k_compute = (
            seq_len * embed_dim * (k_heads * head_size)
        )  # [seq, embed] x [embed, k_heads*head_dim] -> [seq, k_heads*head_dim]
        v_compute = (
            seq_len * embed_dim * (v_heads * head_size)
        )  # [seq, embed] x [embed, v_heads*head_dim] -> [seq, v_heads*head_dim]
    ...
    def profile_mha_score(
        self, seq_len, embed_dim, inner_dim, q_heads, k_heads, v_heads, head_size, kv_cache, pos=1
    ):
        # Profile values
    ...
    def profile_mha_attn(
        self, seq_len, embed_dim, inner_dim, q_heads, v_heads, head_size, kv_cache, pos=2
    ):
    ...
    def profile_slm(self, input_len, inner_dim, kv_cache):
        ...
        self.profile_qkv(
            input_len, self.hidden_dim, self.q_heads, self.kv_heads, self.kv_heads, self.head_size
        )
        self.profile_mha_score(
            input_len,
            self.hidden_dim,
            inner_dim,
            self.q_heads,
