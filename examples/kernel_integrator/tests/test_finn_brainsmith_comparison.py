...
def create_multithreshold_model(channels, input_dt, output_dt, bias=0):
    ...
    thresh = helper.make_tensor_value_info("thresh", TensorProto.FLOAT, [channels, num_thresholds])
    ...
    model.set_tensor_datatype(
        "thresh", DataType["INT16"] if input_dt == "UINT16" else DataType["INT8"]
    )
...
def compare_node_attributes(finn_node, brainsmith_node, report_lines):
    ...
    if brainsmith_node:
        ...
        if finn_node:
            ...
            report_lines.append("\n  Attribute Mapping:")
            report_lines.append(
                f"    Channels: NumChannels={finn_inst.get_nodeattr('NumChannels')} vs CHANNELS={attrs['CHANNELS'].i}"
            )
            report_lines.append(f"    PE: PE={finn_inst.get_nodeattr('PE')} vs PE={attrs['PE'].i}")
            report_lines.append(
                f"    Bias: ActVal={finn_inst.get_nodeattr('ActVal')} vs BIAS={attrs['BIAS'].i}"
            )
...
def generate_and_compare_rtl(finn_model, brainsmith_model, test_name, output_subdir):
    ...
    if finn_model is not None:
        ...
        try:
            ...
            if finn_nodes:
                ...
                report_lines.append(
                    f"  Found node type: {finn_node.op_type}, domain: {finn_node.domain}"
                )
                finn_inst = getCustomOp(finn_node)
                ...
                report_lines.append(
                    f"  Files: {', '.join(finn_files[:5])}{'...' if len(finn_files) > 5 else ''}"
                )

                # Copy wrapper file for easy comparison
                wrapper_candidates = [
                    f for f in finn_files if f.endswith(".v") and "wrapper" in f.lower()
                ]
                ...
                report_lines.append(
                    "  ⚠️ No specialized Thresholding node found after SpecializeLayers"
                )
    ...
    report_lines.append(
        "  Note: RTL generation from examples directory requires manual instantiation"
    )
    ...
    try:
        ...
        try:
            ...
            if not specialized_nodes:
                specialized_nodes = brainsmith_specialized.get_nodes_by_op_type(
                    "thresholding_axi_rtl"
                )
        ...
        if brainsmith_nodes:
            ...
            report_lines.append(
                f"  Found node type: {brainsmith_node.op_type}, domain: {brainsmith_node.domain}"
            )
            ...
            report_lines.append(
                f"  Files: {', '.join(brainsmith_files[:5])}{'...' if len(brainsmith_files) > 5 else ''}"
            )

            # Copy wrapper file for easy comparison
            wrapper_candidates = [
                f for f in brainsmith_files if f.endswith(".v") and "wrapper" in f.lower()
            ]
            if not wrapper_candidates:
            ...
            report_lines.append(
                "  ⚠️ No specialized ThresholdingAxi node found after SpecializeLayers"
            )
    ...
    if results["finn"].get("success") and results["brainsmith"].get("success"):
        ...
        report_lines.append("\nKey Files:")
        report_lines.append(
            f"  Has Verilog wrapper: FINN={finn_has_wrapper}, Brainsmith={brainsmith_has_wrapper}"
        )
        report_lines.append(
            f"  Has SystemVerilog:   FINN={finn_has_sv}, Brainsmith={brainsmith_has_sv}"
        )
...
def run_comprehensive_test(test_name, channels, input_dt, output_dt, bias=0):
    ...
    all_report_lines.append(
        f"Configuration: {channels} channels, {input_dt} -> {output_dt}, bias={bias}"
    )
    ...
    results, rtl_report = generate_and_compare_rtl(finn_model, brainsmith_model, test_name, test_id)
