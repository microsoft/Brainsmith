...
class CleanBertWrapper(nn.Module):
    """Wrapper that forces no attention mask usage for clean export"""
...
def export_quantized_to_onnx(model, output_path, max_length=128):
    ...
    wrapped_model = CleanBertWrapper(model, max_length)
    wrapped_model.eval()
...
def main():
    ...
    original_model = load_fp32_model(args.input_model)
