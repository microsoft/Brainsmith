...
def define_hardware(model, profile, hw_params, dtype):
    ...
    ocm = int(hw_params['ocm'] * hw_params['ocm_util'])
    ...
    # Calculate OCM usage in MB, check for capacity
    ocm_act = activations*dtype
    ocm_weights = weights*dtype + vec_weights*vec_dtype
    if model['offload']:
        ocm_weights *= 2 # Double buffer if offloading
    ocm_used = ocm_act + ocm_weights
    if ocm_used > ocm:
        {'-- OVER!' if (ocm_used > ocm) else ''}
    ...
    print(f"    OCM: {ocm_weights:.2f} weights + {ocm_act:.2f} activations = {ocm_used:.2f}/{ocm/8e6:.2f} MB")
