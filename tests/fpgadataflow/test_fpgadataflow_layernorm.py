...
"""
The above LayerNorm test is not complete, and skips itself. to bridge
the gap in testing, the below test was written, based on other tests
(mainly test_fpgadtaflow_softmax.py). The below test does not have
all of the functionality of the original test.
"""
...
def construct_onnx_model(
    ...
    """
    Builds an ONNX model that contains a single, manually constructed
    FINN LayerNorm node, then wraps it in a QONNX model wrapper, and
    returns it. Assumes the Layernorm's Scale and Bias tensors to be
    filled with ones and zeros, so they'll have no effect on the result.

    TODO: Replace this code. Ideally the HW LayerNorm node should be
          generated by transforming an ONNX FuncLayerNorm node with
          InferLayerNorm(), found in convert_to_hw_layers.py.
    """
    ...
    # Initialisers
    Scale = onnx.numpy_helper.from_array(np.ones(input_shape[-1]), "Scale")
    Bias = onnx.numpy_helper.from_array(np.zeros(input_shape[-1]), "Bias")
    ...
    model = helper.make_model(
        helper.make_graph([layer_norm],                 # Nodes
                          "LayerNorm_Test",             # Name
                          [X],                          # Inputs
                          [Y],                          # Outputs
                          [Scale, Bias]),               # Initialisers
        opset_imports=[OperatorSetIdProto(version=17)]  # ONNX opset
    )
    ...
    for input in model_wrapper.graph.input:
        model_wrapper.set_tensor_datatype(input.name, idt)
...
@pytest.mark.parametrize("ifm_dim", [(1, 128, 384), (1, 12, 12, 128)])
@pytest.mark.fpgadataflow
def test_fpga_dataflow_layernorm_hw_custom_op(
        impl_style:str,
        simd:str,
        idt:str,
        exec_mode:str,
    ...
    """
    This test takes the model generated by construct_onnx_model(), 
    and compares the outputs of execution before and after it is
    transformed to execute via cppsim/rtlsim.The code for this test
    is primarily based on test_fpgadtaflow_softmax.py.
    
    It also compares the expected cycles it takes the layer to execute
    (generated by QONNX) against the cycles the layer takes to execute
    in the rtlsim. Note that cycles testing is only available when the
    test is run in rtlsim.

    Unlike test_fpga_dataflow_layernorm(), this test doesn't infer additional
    elementwise operations, like ElementwiseAdd and ElementwiseMul.
    """
    ...
    io_shape = ifm_dim
    tolerance = 1e-05
    model = construct_onnx_model(impl_style, simd, idt, odt, ifm_dim)
