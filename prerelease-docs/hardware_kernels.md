...
- *Input/Output* - AXI-Stream for activations and weights. A minimum of one input and one output.
...
2. **Prevent Exponential Explosion** - Although there is theoretical value in fully decomposing models to individual operations, this exponentially explodes the design space.
3. **Hand Optimization** - Allows hardware engineers to design kernels hand-optimized on the kernel scale without requiring deep knowledge of the AI model. This allows for hand-optimized performance that fully generated designs lack, while maintaining flexibility by composing the final hardware graph through Brainsmith.
