...
from finnbrainsmith.custom_op.fpgadataflow.hwsoftmax import HWReduceSum
from finn.util.data_packing import npy_to_rtlsim_input, rtlsim_output_to_npy
from finn.util.basic import CppBuilder

class HWReduceSum_hls(HWReduceSum, BS_HLSBackend):
    def __init__(self, onnx_node, **kwargs):
        super().__init__(onnx_node, **kwargs)

    def get_nodeattr_types(self):
        my_attrs = {}
        my_attrs.update(HWReduceSum.get_nodeattr_types(self))
    ...
    def defines(self, var):
        ...
        self.code_gen_dict["$DEFINES$"] = [
            ...
            constexpr unsigned  N = {w};
            using  T = {idtype.get_hls_datatype_str()};
           """
        ]
    ...
    def docompute(self):
        self.code_gen_dict["$DOCOMPUTE$"] = [
            f"""
                ...
                static hls::stream<T>  dst0;

                move(in0_{self.hls_sname()}, src0);
                reduce_sum(src0, dst0);
    ...
    def blackboxfunction(self):
        self.code_gen_dict["$BLACKBOXFUNCTION$"] = [
            ...
            void {self.onnx_node.name}(
                ...
                hls::stream<T> &out_{self.hls_sname()}
                )
            """
        ]
