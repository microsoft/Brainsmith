...
if [ -z "${XILINX_VIVADO}" ]; then
  ...
  if [ -f "${BSMITH_ROOT}/deps/pyxsi/pyxsi.so" ]; then
    gecho "Found pyxsi at ${BSMITH_ROOT}/deps/pyxsi/pyxsi.so"
  else
    OLDPWD=$(pwd)
    cd ${BSMITH_ROOT}/deps/pyxsi
    touch .dockerenv
    make
    cd $OLDPWD
  fi
  export PYTHONPATH=$PYTHONPATH:${BSMITH_ROOT}/deps/pyxsi:${BSMITH_ROOT}/deps/pyxsi/py
...
if [ -f "$HLS_PATH/settings64.sh" ];then
  ...
  yecho "Please note that FINN needs at least version 2020.2 for Vitis HLS support. The recommendation is to use version 2022.2"
  yecho "If you need Vitis HLS, ensure HLS_PATH is set correctly and mounted into the Docker container."
fi

if [ -d "$BSMITH_ROOT/.Xilinx" ]; then
  mkdir "$HOME/.Xilinx"
  if [ -f "$BSMITH_ROOT/.Xilinx/HLS_init.tcl" ]; then
    cp "$BSMITH_ROOT/.Xilinx/HLS_init.tcl" "$HOME/.Xilinx/"
    gecho "Found HLS_init.tcl and copied to $HOME/.Xilinx/HLS_init.tcl"
  else
    yecho "Unable to find $BSMITH_ROOT/.Xilinx/HLS_init.tcl"
  fi

  if [ -f "$BSMITH_ROOT/.Xilinx/Vivado/Vivado_init.tcl" ]; then
    mkdir "$HOME/.Xilinx/Vivado/"
    cp "$BSMITH_ROOT/.Xilinx/Vivado/Vivado_init.tcl" "$HOME/.Xilinx/Vivado/"
    gecho "Found Vivado_init.tcl and copied to $HOME/.Xilinx/Vivado/Vivado_init.tcl"

  else
    yecho "Unable to find $BSMITH_ROOT/.Xilinx/Vivado/Vivado_init.tcl"
  fi
else
